---
title: "Roullete analysis-Under construction"
...
In this project we will calculate many important things about the game of roullete, and it will be proven in general that no-one can win an unfair game with collecting only probabilistic information and the past outcomes of the game.
Familiarity with stochastic mathematics and the game of roullete is assumed. 

## Bet Analysis

In this section, many roullete bets will be analyzed, and simulations will be done in order to understand the effect of each bet on the long-run. The parameters that we care about are the time of ruin(e.g. the time when the capital reaches zero or a value that cannot support this type of bet) and the exit capital(e.g. the ammount of units the player has when he finishes playing a specific number of rounds).

### Single number bet
We first turn our attention at one of the most basic bets availiable, that is, betting a chip at a single number. We know that the capital of the player at round n is
...
It is large, having in mind that we bet 1 unit.
...
With the above term, I mean all the bets that if won, they give the player a profit that equals they
initial bet.
...
### Jeu Zero
The capital in this kind of bet is 
...
Here, the variance is $Var(X_n|\mathcal{F}_{n-1}) = \frac{4}{37}(X_{n-1} + 14)^2 + \frac{1}{37}(X_{n-1} + 32)^2 + \frac{3}{37}(X_{n-1} + 8)^2 + \frac{29}{37} (X_{n-1} - 4)^2 - (X_{n-1} - \frac{4}{37})^2 = \frac{2448}{37} \approx 66.162$, which is again large.

Like before, the following graphs give a general picture of this kind of bet. It is obvious since Jeu Zero has the less favorable long-term profit, that most players get ruined or have less capital when the game ends.
...
The choice of betting strategy depends on how risk averse a player is, but no one can hope to win in the long term, using statistical information. 
However, one can use a strategy which I call "Exploiting the statisical limit" to minimize the losses. This is based on the following logical assumption: 

When a probabilistic phenomenon is happening, and a quantiny of some nature is being observed(e.g. how much time someone loses in traffic jams each day), the empirical distribution must converge to the theoretical(we assume here a frequentist approach). 
...
Based on the above, suppose we have a player that observes 480,000 spins, notes the number that happens to arise less often and bets ove chip on this number for the following 20,000 spins. The simulation below show that even though this is not a way to win, this strategy show much better results than just betting in a random number for 20,000 spins.
...
We see thath the mean value of the profit(when playing 20,000 rounds) of the single number strategy is `r mplein` and that of the "Exploiting statistical limit" strategy is `r mprof`.

There are attempts to build a gambling strategy based on mechanical information of the wheel, which if made under cover, can make roullete a profitable game. These methods require knowledge of mechanics, so it will not be discussed here.
...
But there exist something else we must turn our attention to. Imagine that a player simulates 10,000 games of roullete, with 500 rounds each. Then makes an histogram of the max value that the capital reached before the end, and uses the following strategy.

He starts playing, and if the capital reaches the mean value of the maximums, he stops. Hence, he avoids the games where after the maximum, the capital shrinks.

We will now prove that this is again not a winning strategy.
